...
export function CollaborationHub({ workId }: { workId: string }) {
  ...
  async function merge(contributionId: string) {
    ...
    if (!(await confirmAction({
      title: 'Aceitar e mesclar contribuição?',
      description: 'O conteúdo aprovado será aplicado à obra. Revise a proposta antes de continuar.',
      confirmLabel: 'Aceitar e mesclar',
    }))) return
    setBusy(true)
  ...
  if (!hub) return <><NovaHeader title="Contribuições" /><main className="collaboration-page"><div className="profile-not-found"><span><NovaIcon name="branch" size={28} /></span><h1>{message || 'Colaboração indisponível.'}</h1>{user ? <Link className="primary-button" href={`/works/${workId}`}>Voltar à obra</Link> : <button className="primary-button" onClick={login}>Entrar</button>}</div></main></>
