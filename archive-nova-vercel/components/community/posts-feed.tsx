      ...
      <main className="community-page">
        ...
        {!loading && posts.length ? <section className="post-feed">{posts.map((post) => {
          ...
          return <article className="community-post" key={post.id}>
            ...
            {post.image_urls.length ? <div className={`post-image-grid count-${post.image_urls.length}`}>{post.image_urls.map((url, index) => <a href={url} target="_blank" rel="noreferrer" key={url}><img src={url} alt={`Imagem ${index + 1} do post`} /></a>)}</div> : null}
            ...
            <footer className="post-actions"><button className={post.liked ? 'active' : ''} onClick={() => void toggleLike(post)}>♥ <span>{post.likes_count}</span></button><button onClick={() => void loadComments(post.id)}>☁ <span>{post.comments_count}</span></button><button onClick={() => { void navigator.clipboard?.writeText(`${window.location.origin}/posts#${post.id}`); setMessage('Link do post copiado.') }}>↗ Compartilhar</button></footer>
        ...
        {!loading && !posts.length ? <div className="studio-empty large"><span>☁</span><h2>Nenhum post por aqui ainda</h2><p>Quando a comunidade publicar novidades, elas aparecerão aqui.</p></div> : null}
      </main>
    </>
  )
}
