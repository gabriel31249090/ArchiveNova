...
export function BetaReader({ token }: { token: string }) {
  ...
  return <main className="beta-reader-v49">
    ...
    <div className="beta-reader-layout"><aside><p className="eyebrow">Capítulos</p>{draft.chapters.map((item) => <button type="button" className={item.id === chapter?.id ? 'active' : ''} key={item.id} onClick={() => setChapterId(item.id)}><b>{String(item.position).padStart(2,'0')}</b><span>{item.title || `Capítulo ${item.position}`}</span><small>{Number(item.word_count || 0).toLocaleString('pt-BR')} p.</small></button>)}</aside><article className="beta-reader-paper"><div><p className="eyebrow">Capítulo {chapter?.position}</p><h1>{chapter?.title || 'Sem tíulo'}</h1><section dangerouslySetInnerHTML={{ __html: chapter?.content_html || '<p></p>' }} /></article><aside className="beta-feedback-panel"><p className="eyebrow">Feedback privado</p><h2>Deixe comentários para o autor.</h2><label>Trecho de referência<input value={anchor} onChange={(e) => setAnchor(e.target.value)} placeholder="Opcional: copie uma frase" /></label><label>Seu comentário<textarea rows={8} value={feedback} onChange={(e) => setFeedback(e.target.value)} placeholder="O que funcionou? Onde ficou confuso/ Alguma ideia?" /></label><button className="primary-button" type="button" onClick={() => void sendFeedback()}>Enviar feedback</button>{notice ? <p className="beta-notice">{notice}</p> : null}</aside></div>
  </main>
}
