...
import { NovaShieldPanel } from '@/components/moderation/novashield-panel'

function reasonLabel(reason: string) {
  return ({ HARASSMENT: 'Assédio', HATE: 'Discurso de ódio', SPAM: 'Spam', PLAGIARISM: 'Plágio', ILLEGAL: 'Conteúdo ilegal', NOVASHIELD: 'NovaShield', OTHER: 'Outro' } as Record<string, string>)[reason] || reason
}
  ...
  return (
    <>
      ...
      <main className="moderation-page">
        ...
        <div className="moderation-ops-links"><Link className="secondary-button" href="/sac/staff"><NovaIcon name="help" size={16}/> Abrir NovaCare Staff</Link></div>
        <NovaShieldPanel />
        <section className="moderation-human-queue-head"><div><p className="eyebrow">Denúncias e ações</p><h2>Fila humana</h2><p>Relatos de usuários e alertas elevados pelo NovaShield aparecem juntos aqui.</p></div></section>
        {reports.length ? <div className="moderation-list">{reports.map((report) => <article className="moderation-card" key={report.id}><header><div className="moderation-reason"><span>{reasonLabel(report.reason)}</span><b className={report.status.toLowerCase()}>{report.status === 'REVIEWING' ? 'Em revisão' : 'Nova'}</b></div><small>{new Intl.DateTimeFormat('pt-BR', { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(report.created_at))}</small></header><div className="moderation-target"><span>{report.work_id ? 'OBRA' : 'COMENTÁRIO'}</span>{report.work_id ? <><Link href={`/works/${report.work_id}`}><h2>{report.work_title || 'Obra denunciada'}</h2></Link></> : <><h2>Comentário de @{report.comment_author_username || 'usuário'}</h2><blockquote>{report.comment_body || 'Comentário indisponível.'}</blockquote></>}</div>{report.details ? <div className="moderation-details"><strong>{report.reason === 'NOVASHIELD' ? 'Alerta interno do NovaShield' : 'Relato de @' + (report.reporter_username || 'usuário')}</strong><p>{report.details}</p></div> : <div className="moderation-details"><strong>Relato</strong><p>O usuário não acrescentou detalhes.</p></div>}<footer>{report.work_id ? <button className="danger-button" disabled={busyId === report.id} onClick={() => void hideWork(report)}>Ocultar obra</button> : <button className="danger-button" disabled={busyId === report.id} onClick={() => void hideComment(report)}>Ocultar comentário</button>}<div><button className="ghost-button" disabled={busyId === report.id} onClick={() => void setStatus(report, 'DISMISSED')}>Descartar</button>{report.status === 'OPEN' ? <button className="secondary-button" disabled={busyId === report.id} onClick={() => void setStatus(report, 'REVIEWING')}>Assumir revisão</button> : null}<button className="primary-button" disabled={busyId === report.id} onClick={() => void setStatus(report, 'RESOLVED')}>Resolver</button></div></footer></article>)}</div> : <div className="studio-empty moderation-empty"><span><NovaIcon name="check" size={30} /></span><h2>Fila limpa</h2><p>Não há denúncias abertas ou em revisão.</p></div>}
        {confirmDialog}
      </main>
    </>
  )
