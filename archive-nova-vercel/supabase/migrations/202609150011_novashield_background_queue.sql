-- ArchiveNova v4.8 — automatic NovaShield background queue

alter table public.works
  add column if not exists moderation_scan_failures smallint not null default 0
    check (moderation_scan_failures between 0 and 10),
  add column if not exists moderation_scan_error text;

create index if not exists works_novashield_background_idx
  on public.works(moderation_scan_pending, moderation_scan_failures, updated_at)
...
begin
  ...
  if uid is null and upper(coalesce(scan_source,'')) <> 'BACKGROUND' then
    raise exception 'NO_ACCESS' using errcode='42501';
  end if;
  if uid is not null and work_row.creator_id<>uid and not public.is_staff() then
    raise exception 'NO_ACCESS' using errcode='42501';
  end if;
  ...
  set moderation_risk_score=total_score,
      ...
      moderation_scan_failures=0,
      moderation_scan_error=null,
...
create or replace function public.novashield_process_pending(batch_size integer default 6)
returns jsonb
language plpgsql
security definer
set search_path=public,auth,pg_temp
...
begin
  for item in
    ...
    where w.deleted_at is null
      and w.status <> 'DRAFT'
      and w.moderation_scan_pending
      and w.moderation_scan_failures < 3
    order by w.updated_at asc
    limit greatest(1,least(coalesce(batch_size,6),20))
    for update skip locked
  loop
    begin
      perform public.novashield_scan_work(item.id,'BACKGROUND');
      processed_count:=processed_count+1;
    exception when others then
      failed_count:=failed_count+1;
      update public.works
      set moderation_scan_failures=least(10,moderation_scan_failures+1),
          moderation_scan_error=left(sqlerrm,1200)
      where id=item.id;
    end;
  end loop;
...
revoke execute on function public.novashield_process_pending(integer) from public,anon,authenticated;
...
select cron.schedule(
  'archivenova-novashield-pending',
  '*/2 * * * *',
  'select public.novashield_process_pending(6)'
);
